...
/**
 *  A scanner take sentences as a String instance, recognize each mark, letter number, Arabic number and word.
 *  Then put tokens in sentence clauses (ArrayList<TokenGeneralKind>), and generate connective operators for those clauses.
 */
public class Scanner {
	...
	/**
	 * Organize each token: change upper case to lower case; 
	 * 						escape meaningless mark; 
	 * 						recognize Arabic number in general number types;
	 * 						split Arabic number from letters. 
	 * 
	 * @param sentenceArray
	 * 			an Array of tokens in String type
	 * 
	 * @return an ArrayList of tokens in types that mixed by String and SpecialToken
	 */
	...
	/**
	 * Recognize word in String-type left in the ArrayList by using FixMachine methods,
	 * calculate multiple weight of each word regarding core-word, prefix and suffix,
	 * and find out money-number in the list then change the property-type into NumberProperties.MONEYNUMBER.
	 * 
	 * @param sentenceArray
	...
	{	FixMachine fixmachine = new FixMachine();
		...
		{	if( sentenceArray.get(i) instanceof String)
			{	String instanceofString = (String)sentenceArray.get(i);
				...
				result.add(new Token((String)sentenceArray.get(i),
				coreMeaning,property,weight,functionType));
			}
			else
		...
		{
			String tokenImage = ((TokenGeneralKind)newResult.get(index)).getImage();
			if (( tokenImage.equals("to") || tokenImage.equals("from")) &&  index + 1 < newResult.size() 
					&& newResult.get(index+1) instanceof Token 
					&& ((CoreMeaning)((Token)newResult.get(index+1)).getCoreMeaning()) == CoreMeaningOne.NUMBERTYPE)
			{
		...
		{	if(newResult.get(i) instanceof Token && ((Token)newResult.get(i)).getImage().equals("$"))
			{	if(newResult.get(i+1) instanceof Token && ((Token)newResult.get(i+1)).getCoreMeaning() == CoreMeaningOne.NUMBERTYPE)
				{((Token)newResult.get(i+1)).setProperty(NumberProperties.MONEYNUMBER);
				}
			}
			else if(newResult.get(i) instanceof Token && ((Token)newResult.get(i)).getFunctionType() == ServiceProperty.COST
					&& ((Token)newResult.get(i)).getWeight() == 0F
					&& ((Token)newResult.get(i)).getProperty() == Properties.NOUN)
			{	if( i-1 >=0 && newResult.get(i-1) instanceof Token && ((Token)newResult.get(i-1)).getCoreMeaning() == CoreMeaningOne.NUMBERTYPE)
				{((Token)newResult.get(i-1)).setProperty(NumberProperties.MONEYNUMBER);
				}
			}
			else if( i >1 && i+1 < newResult.size()
					&& newResult.get(i) instanceof Token && ((Token)newResult.get(i)).getImage().equals("to")
					&& (newResult.get(i-1) instanceof Token && ((Token)newResult.get(i-1)).getCoreMeaning() == CoreMeaningOne.NUMBERTYPE)
					&& ( (newResult.get(i+1) instanceof Token && ((Token)newResult.get(i+1)).getCoreMeaning() == CoreMeaningOne.NUMBERTYPE)
							||(newResult.get(i+1) instanceof Token && ((Token)newResult.get(i+1)).getFunctionType() == ServiceProperty.COST
	...
	/**
	 * Generate tokens into clause by conjunction marks or words,
	 * and put the connective operator between clauses adjective to each other.
	 * 
	 * @param tokens
	...
	{	int clauseStartIndex = 0;
		...
		{	if(tokens.get(i) instanceof Token 
				&& (((Token)tokens.get(i)).getProperty() == Properties.CCONJ 
					|| ((Token)tokens.get(i)).getProperty() == Properties.RPREP) && !((Token)tokens.get(i)).getImage().endsWith("&"))
			{	Token targetToken = (Token) tokens.get(i);
				ArrayList<TokenGeneralKind> copy = (ArrayList<TokenGeneralKind>)tokens;
				ArrayList<TokenGeneralKind> newClause =  new ArrayList<TokenGeneralKind> (copy.subList(clauseStartIndex, i));
			...
			{	if(i+1 <tokens.size() && tokens.get(i-1) instanceof Token && tokens.get(i+1) instanceof Token)
				{	Token tokenBeforeAnd = (Token)tokens.get(i-1);
					...
					//when the two token besides "and" have same property and function type  then and will not work as a conjunctive.
			...
			{	Token targetToken = (Token) tokens.get(i);
				...
				{	ArrayList<TokenGeneralKind> newClause =  new ArrayList<TokenGeneralKind>(tokens.subList(clauseStartIndex, i));
					clauseStartIndex = i+1;
					resultRequestClause.add(newClause);
		...
		// put the rest of tokens without any conjunction with it inside clauseArray as a clause
		 if(clauseStartIndex < tokens.size() )
		{
			ArrayList<TokenGeneralKind> newClause =  new ArrayList<TokenGeneralKind>(tokens.subList(clauseStartIndex, tokens.size()));
			clauseStartIndex = tokens.size() -1;
			resultRequestClause.add(newClause);
		}
		 //remove the last period recognized as conjunction out of operatorBetweenClauses
	...
	private ArrayList<TokenGeneralKind> preFixDateNumber(ArrayList<TokenGeneralKind> clause)
	{
		...
		for(int index = 0; index < clause.size(); index ++)
		{
			...
			if( clause.get(index) instanceof Token 
					...
					&& ((CoreMeaningOne)((Token)clause.get(index)).getCoreMeaning()).getBasicProperty() == Properties.MONTH
					 && ((CoreMeaningOne)((Token)clause.get(index)).getCoreMeaning()).getBasicFunctionType() == ServiceProperty.LANDA
					 )
			{
				int dateInInt = (int)(((CoreMeaningOne)((Token)clause.get(index))
						.getCoreMeaning()).getMultipleWeight());
				date =  String.valueOf(dateInInt) + "/";
				if(index + 1 < clause.size()
						&& ((Token)clause.get(index+1)).getCoreMeaning() instanceof CoreMeaningOne
						&&  ((CoreMeaningOne)((Token)clause.get(index+1)).getCoreMeaning()) == CoreMeaningOne.NUMBERTYPE )
				{
					...
					if(dateNumber > 31 && ((date.equals("1") ||date.equals("3") ||date.equals("5") ||
						...
						dateNumber = 31;
					else if(dateNumber > 30 && ((date.equals("4") ||date.equals("6") ||date.equals("9") ||
								date.equals("11") ))) 
						dateNumber = 30;
					...
					if(index + 2 < clause.size()
							...
							&&  ((CoreMeaningOne)((Token)clause.get(index+2)).getCoreMeaning()) == CoreMeaningOne.NUMBERTYPE )
					{
				...
				{
					String dateNumber="";
					if( (date.equals("1") ||date.equals("3") ||date.equals("5") ||
							date.equals("7") ||date.equals("8") ||date.equals("10") ||date.equals("12")))
						dateNumber = "31";
					else if( (date.equals("4") ||date.equals("6") ||date.equals("9") ||
							date.equals("11") )) 
						dateNumber = "30";
	...
	{	int indexOfNumberOccer = hasNumber(token);
		...
		{	int indexAtHere = -1;
			...
			{	numberWord += String.valueOf(token.charAt(i));
				if(i == token.length() -1)
				{	seperateNumberOutputArray.add(new Token(numberWord, CoreMeaningOne.NUMBERTYPE,NumberProperties.REGULARNUMBER));
					return seperateNumberOutputArray;
				}
			}
	//when '.' works as a radix point			
			...
			{	indexAtHere = i+1;
				...
				while(indexAtHere <  token.length() &&(isNumber(token.charAt(indexAtHere)) 
						|| token.charAt(indexAtHere)=='%' 
						|| isMeaningfulSpecicalToken(token.charAt(indexAtHere))) )
				{	numberWord += String.valueOf(token.charAt(indexAtHere));
					...
					{
					seperateNumberOutputArray.add(new Token(numberWord.substring(0, numberWord.length()-1), CoreMeaningOne.NUMBERTYPE,NumberProperties.REGULARNUMBER));
	...
	//put it in array when when ':' use as time number separator
			...
			{	
				...
				{	numberWord = numberWord+  ":"+ String.valueOf(token.charAt(i+1));
					...
					seperateNumberOutputArray.add(new Token(numberWord, CoreMeaningOne.NUMBERTYPE,NumberProperties.TIMENUMBER));
					if(i+3 < token.length())
					...
					if(i+1 < token.length()&& token.charAt(i+1) == '/')
						{	if(i+5 < token.length() && isNumber(token.charAt(i+2))&& isNumber(token.charAt(i+3))
							...
							{	numberWord = numberWord+  "/"+ token.substring(i+2, i+6);
								seperateNumberOutputArray.add(new Token(numberWord, CoreMeaningOne.NUMBERTYPE,NumberProperties.DATENUMBER));
								if(i+6 < token.length())
									seperateNumberOutputArray.add(token.substring(i+6, token.length()));
							}
							else if(i+3 < token.length() && isNumber(token.charAt(i+2))&& isNumber(token.charAt(i+3)))
	...
	{	ArrayList<TokenGeneralKind> newTokenObject = new ArrayList<TokenGeneralKind>();
		...
		{	
			...
			{	int j = 1;
				...
				while(i+j < tokens.size() && tokens.get(i+j) instanceof Token && ((Token)tokens.get(i+j)).getCoreMeaning() instanceof CoreMeaning
						&& (((CoreMeaning)((Token)tokens.get(i+j)).getCoreMeaning()).getBasicProperty() == Properties.NUMBER
						|| (((CoreMeaning)((Token)tokens.get(i+j)).getCoreMeaning()).getImage() == "and")))
				{	String targetImage = ((CoreMeaning)((Token)tokens.get(i+j)).getCoreMeaning()).getImage();
					if(targetImage.equals("hundred") ||targetImage.equals("thousand") || targetImage.equals("million")|| targetImage.equals("billion"))
				...
				startIndex = i;
				numberAmount += midAmount;
				newTokenObject.add(new Token(String.valueOf(numberAmount), CoreMeaningOne.NUMBERTYPE,NumberProperties.REGULARNUMBER));
			}
...
//					&& (((Token)tokens.get(i)).getFunctionType() == ServiceProperty.LANDA))
//			{
//				String StringOfDate = String.valueOf(((Token)tokens.get(i)).getWeight());
//				if(i+1 < tokens.size() 
//						&&  ((Token)tokens.get(i+1)).getCoreMeaning() instanceof CoreMeaning
//						&& (((Token)tokens.get(i)).getProperty() == Properties.NOUN)
//						&& (((Token)tokens.get(i)).getFunctionType() == ServiceProperty.LANDA)
//						&&(((Token)tokens.get(i)).getWeight() != WeightOriginalRange.STABLE.getWeight()) )
//				{
////					StringOfDate = StringOfDate + "/" + String.valueOf(((Token)tokens.get(i)).getWeight());
...
//			}
	...
	{	if(c == '.')
			...
			return new Token("$", SpecialToken.DOLLAR,Properties.NUMBER);
		else if(c == '>')
	...
	{	this.words = getArrayFromStringInput(this.inputSentence) ;
		...
		{	for(TokenGeneralKind oo : clauseArray.get(i))
				{if(oo instanceof Token)
					System.out.print(((Token)oo).getImage() + "(" + ((Token)oo).getProperty() + ")(" + ((Token)oo).getWeight() + ")     ");
