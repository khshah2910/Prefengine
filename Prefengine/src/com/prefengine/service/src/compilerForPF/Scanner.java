...
public class Scanner {
	...
	{	FixMachine fixmachine = new FixMachine();
		...
		{	if(newResult.get(i) instanceof Token && ((Token)newResult.get(i)).getImage().equals("$"))
			{	if(newResult.get(i+1) instanceof Token && ((Token)newResult.get(i+1)).getCoreMeaning() == CoreMeaning.NUMBERTYPE)
			...
			{	if( i-1 >=0 && newResult.get(i-1) instanceof Token && ((Token)newResult.get(i-1)).getCoreMeaning() == CoreMeaning.NUMBERTYPE)
				{((Token)newResult.get(i-1)).setProperty(NumberProperties.MONEYNUMBER);
				}
			}
			else if( i >1 && i+1 < newResult.size()
					&& newResult.get(i) instanceof Token && ((Token)newResult.get(i)).getImage().equals("to")
					&& (newResult.get(i-1) instanceof Token && ((Token)newResult.get(i-1)).getCoreMeaning() == CoreMeaning.NUMBERTYPE)
					&& ( (newResult.get(i+1) instanceof Token && ((Token)newResult.get(i+1)).getCoreMeaning() == CoreMeaning.NUMBERTYPE)
	...
	{	int indexOfNumberOccer = hasNumber(token);
		...
		if(token.length() == 1)
		{	seperateNumberOutputArray.add(new Token(token, CoreMeaning.NUMBERTYPE,NumberProperties.REGULARNUMBER));
		...
		{	int indexAtHere = -1;
			...
			{	numberWord += String.valueOf(token.charAt(i));
				if(i == token.length() -1)
				{	seperateNumberOutputArray.add(new Token(numberWord, CoreMeaning.NUMBERTYPE,NumberProperties.REGULARNUMBER));
					return seperateNumberOutputArray;
				}
			}
	//when '.' works as a radix point			
			...
			{	indexAtHere = i+1;
				...
				{	numberWord += String.valueOf(token.charAt(indexAtHere));
					...
					{
					seperateNumberOutputArray.add(new Token(numberWord.substring(0, numberWord.length()-1), CoreMeaning.NUMBERTYPE,NumberProperties.REGULARNUMBER));
	...
	//put it in array when when ':' use as time number separator
			...
			{	
				...
				{	numberWord = numberWord+  ":"+ String.valueOf(token.charAt(i+1));
					...
					seperateNumberOutputArray.add(new Token(numberWord, CoreMeaning.NUMBERTYPE,NumberProperties.TIMENUMBER));
					if(i+3 < token.length())
					...
					if(i+1 < token.length()&& token.charAt(i+1) == '/')
						{	if(i+5 < token.length() && isNumber(token.charAt(i+2))&& isNumber(token.charAt(i+3))
							...
							{	numberWord = numberWord+  "/"+ token.substring(i+2, i+6);
								seperateNumberOutputArray.add(new Token(numberWord, CoreMeaning.NUMBERTYPE,NumberProperties.DATENUMBER));
								if(i+6 < token.length())
	...
	{	ArrayList<TokenGeneralKind> newTokenObject = new ArrayList<TokenGeneralKind>();
		int startIndex = 0;
		for(int i = 0;i< tokens.size();i++)
		{	if(i< tokens.size() && tokens.get(i) instanceof Token && ((Token)tokens.get(i)).getCoreMeaning() instanceof CoreMeaning
			...
			{	int j = 1;
				...
				startIndex = i;
				numberAmount += midAmount;
				newTokenObject.add(new Token(String.valueOf(numberAmount), CoreMeaning.NUMBERTYPE,NumberProperties.REGULARNUMBER));
			}
		}
	...
	{	if(c == '.')
			...
			return new Token( "",CoreMeaning.NUMBERTYPE,NumberProperties.REGULARNUMBER);
		else
			return null;					
	}
